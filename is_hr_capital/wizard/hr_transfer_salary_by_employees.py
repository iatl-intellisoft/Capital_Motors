...
class HrSalaryTransfer(models.TransientModel):
    _name = 'hr.salary.transfer'
    ...
    employee_ids = fields.Many2many('hr.employee', 'hr_employee_rel', 'transfer_id', 'employee_id', 'Employees',
    ...
    def _check_undefined_slots(self, work_entries, payslip_run):
        ...
        for contract, work_entries in work_entries_by_contract.items():
            ...
            if outside:
                raise UserError(
                    _("Some part of %s's calendar is not covered by any work entry. Please complete the schedule.") % contract.employee_id.name)
