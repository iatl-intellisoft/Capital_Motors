...
class emp_support(models.Model):
    ...
    def create(self, vals_list):
        res = super(emp_support, self).create(vals_list)
        debit_line_vals = {
            'name': res.item,
            'debit': res.amount,
            'credit': 0.0,
...
class office_maintenance(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        res = super(office_maintenance, self).create(vals_list)
        rec_vals = {
            'fa_date': res.date,
...
class HR_attendance(models.Model):
    ...
    check_in_date = fields.Date(
        "Date", compute='check_in_without_time', store=True)
...
class saturday_work(models.Model):
    ...
    def create_penalty(self):
        ...
        saturday_work_ids = self.env['saturday.work'].search(
            [('date', '=', today_date)])
...
class hr_con(models.Model):
    ...
    contract_period = fields.Float("contract period", digits=(
        16, 4), compute='compute_contract_period')

    def compute_contract_period(self):
        for rec in self:
            contract_period = 0.0
            contract_period = str(
                (datetime.strptime(rec.date_start) - datetime.strptime(rec.date_end)).days)
