...
class CheckFollowups(models.Model):
    ...
    @api.model
    def get_loan_paid(self):
        manager_group_id = self.env.ref('account.group_account_manager').id
        con_ids = self.search([])
        for rec in con_ids:
            if rec.contract_end:
                today = datetime.strptime(str(fields.Date.today()), '%Y-%m-%d')
                alert_date = datetime.strptime(str(rec.Date), '%Y-%m-%d')
                days = (today - alert_date).days
                if days == 0:
                    activity = self.env['mail.activity.type'].search([('name', 'like', 'Email')],
                                                                     limit=1)
                    self.env.cr.execute(
                    ...
                    for mg in self.env.cr.fetchall():
                        vals = {
                            ...
                            'res_id': rec.id,
                            'res_model_id': rec.env['ir.model'].search([('model', 'like', 'check_followups.check_followups')],
                                                                       limit=1).id,
                            'user_id': mg[0] or 1,
                            'summary': 'check due date is today' + rec.Date,
                        }
                        print(vals)
                    # add lines
