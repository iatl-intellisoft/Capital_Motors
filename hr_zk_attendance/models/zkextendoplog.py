def zkextendoplog(self, index=0):
    ...
    data_seq = [self.data_recv.encode(
        "hex")[4:6], self.data_recv.encode("hex")[6:8]]

    if index == 0:
        self.data_seq1 = hex(
            int(data_seq[0], 16) + int('104', 16)).lstrip("0x")
        self.data_seq2 = hex(int(data_seq[1], 16) + int('19', 16)).lstrip("0x")
        desc = ": +104, +19"
        header = "0b00"
    elif index == 1:
        self.data_seq1 = hex(
            abs(int(data_seq[0], 16) - int('2c', 16))).lstrip("0x")
        self.data_seq2 = hex(
            abs(int(data_seq[1], 16) - int('2', 16))).lstrip("0x")
        desc = ": -2c, -2"
        header = "d107"
    elif index >= 2:
        self.data_seq1 = hex(
    ...
    if len(self.data_seq1) >= 3:
        self.data_seq2 = hex(int(self.data_seq2, 16) +
                             int(self.data_seq1[:1], 16)).lstrip("0x")
        self.data_seq1 = self.data_seq1[-2:]

    if len(self.data_seq2) >= 3:
        self.data_seq1 = hex(int(self.data_seq1, 16) +
                             int(self.data_seq2[:1], 16)).lstrip("0x")
    ...
    data = header+self.data_seq1+self.data_seq2 + \
        self.id_com+counter+"00457874656e644f504c6f6700"
