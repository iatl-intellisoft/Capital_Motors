...
class CheckFollowups(models.Model):
    ...
    def action_cancelc(self):
        self.Last_state = self.state
        self.write({'state': 'cancelc'})
        return True
    ...
    def make_move(self):
        for r in self:
            if r.payment_id:
                ...
                debit, credit, ss, amount_currency = aml.with_context(date=r.payment_id.date).compute_amount_fields(r.amount, r.payment_id.currency_id, r.payment_id.company_id.currency_id)
    ...
    def _get_move_vals(self, move_date):
        ...
        if self.payment_id:
            return {
                'date': move_date,
                'ref': str('Check No/ ') + str(self.check_no),
    ...
    def _get_move_line_accounts(self):
        ...
        if self.type == 'inbound':
            ...
            if self.state == 'in_bank' and self.Last_state == 'under_collection':
                return self.to_account_journal_id.default_account_id.id, self.payment_id.journal_id.company_id.account_journal_payment_debit_account_id.id
            if self.state == 'rdc' and self.Last_state == 'under_collection':
                return self.payment_id.journal_id.rdc.id, self.payment_id.journal_id.company_id.account_journal_payment_debit_account_id.id
            elif self.state == 'under_collection' and self.Last_state == 'rdc':
                return self.payment_id.journal_id.company_id.account_journal_payment_debit_account_id.id, self.payment_id.journal_id.rdc.id
                ...
                return self.account_holder.property_account_receivable_id.id, self.payment_id.journal_id.company_id.account_journal_payment_debit_account_id.id
            ...
            if self.payment_id:
                if self.state == 'withdrawal' and self.Last_state == 'out_standing':
                    return self.payment_id.journal_id.company_id.account_journal_payment_credit_account_id.id, self.payment_id.journal_id.default_account_id.id
                elif self.state == 'rdv' and self.Last_state == 'out_standing':
                    return self.payment_id.journal_id.company_id.account_journal_payment_credit_account_id.id, self.payment_id.journal_id.rdv.id
                    ...
                    return self.payment_id.journal_id.company_id.account_journal_payment_credit_account_id.id, self.payment_id.partner_id.property_account_payable_id.id
                else:
                    _logger.error('can not determine move accounts for {} with state = {}, Last_state = {}. this is unknown change in the state!'.format(self, self.state, self.Last_state))
                    raise ValidationError("Unknown check state changes!\nFrom '{}' to '{}'".format(
                        self.Last_state or '', self.state or ''
...
class Partner(models.Model):
    ...
    def action_view_checks(self):
        ...
        action = self.env.ref('ii_check_management_15.check_followups_customer').read()[0]
